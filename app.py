...
def generate_unique_token():
    while True:
        ...
        # Check if the token is unique in the database (reset_token and verification_token)
        is_unique = users_collection.count_documents({
            "$or": [
                {"reset_token": token},
...
def generate_unique_address():
    ...
    while True:
        ...
        address = prefix + random_chars
        
        # Check if the address is unique in the database
        if users_collection.count_documents({"vc_id": address}) == 0:
            return address
...
def verify_email(token):
    ...
    if user:
        users_collection.update_one({'_id': user['_id']}, {'$set': {'email_verified': True, 'verification_token': ''}})
...
def make_admin(username):
    if 'username' in session:
        ...
        current_user = user_session_check()
...
def admin_balance_locked():
    ...
    if 'username' in session:
        ...
        if current_user['admin']:
            ...
            for users in user:
                vc_id = users['vc_id']
                vc_balance_locked_data = users.get('vc_balance_locked', []) 
            ...
            return render_template('admin_balance_locked.html',is_admin=current_user.get('admin', False), balances=formatted_balances, is_logged_in=is_logged_in, user=current_user)
        else:
            flash('Access Denied. You are not authorized to view this page.', 'danger')
            return redirect(url_for('login'))
    else:
